...
namespace BLL
{
    public static class UserService
    {
        ...
        public static bool ConfirmToken(ChangePassword changePassword)
        {
            try
            {
                if (changePassword.SendingDate.AddMinutes(10).ToLocalTime() >= DateTime.Now)
                {
        ...
        private static bool AddChangePassword(int userId, string token)
        {
            try
            {
                string query = "INSERT INTO task_management.change_password(user_id, token, sending_date,attemp_num) " +
                    $"VALUES('{userId}', '{token}', {BaseService.FormatDate(DateTime.Now, "yyyy-MM-dd HH:mm:ss")}, 1);";
