    ...
    public static class PresenceHoursService
    {
        ...
        public static List<dynamic> GetPresenceStatusPerWorkers(int teamLeaderId)
        {
            string query =
                ...
                "AND pre.project_id = w.project_id " +
                "AND pre.worker_id = u.user_id " +
                "AND pre.worker_id = w.worker_id " +
                $"WHERE u.team_leader_id = {teamLeaderId} " +
                //$"AND(MONTH(start_hour) is null or MONTH(start_hour) = MONTH(CURRENT_DATE())) " +
        ...
        public static List<dynamic> GetPresenceStatusPerProjects(int workerId)
        {
          ...
          "LEFT JOIN task_management.presence_hours pre " +
          $"ON pro.project_id = pre.project_id AND pre.worker_id = {workerId} " +
        ...
        public static List<PresenceHours> GetPresenceHours(int workerId, int projectId)
        {
            try
            {
                ...
                return presenceHoursList.ToList();//.Where(presenceHours => presenceHours.StartHour.Month == DateTime.Today.Month).ToList();
            }
            catch (Exception ex)
            {
                throw ex;
