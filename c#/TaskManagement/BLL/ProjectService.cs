    ...
    public static class ProjectService
    {
        public static bool AddProject(Project newProject)
        {
            try
            {
                ...
                //{
                //    query += $"INSERT INTO task_management.worker_hours(project_id,worker_id,num_hours,is_complete) VALUES (@project_id,{worker.UserId},0,1);";
                //});
                query += "COMMIT;";
        ...
        public static bool UpdateProject(Project project)
        {
            try
            {
                ...
                query += $"UPDATE task_management.project " +
                    $"SET team_leader_id={project.TeamLeaderId}, " +
                    $"total_hours={project.TotalHours}, " +
                    $"end_date={BaseService.FormatDate(project.EndDate)}, " +
                    $"is_complete={project.IsComplete} " +
                    $"WHERE project_id={project.ProjectId};";
                //add the divided hours for each department
                foreach (DepartmentHours departmentHour in project.DepartmentsHours)
                {
                    query += $"UPDATE task_management.department_hours " +
                        $"SET department_hours.num_hours={departmentHour.NumHours} " +
                        $"WHERE project_id={project.ProjectId} AND department_id={departmentHour.DepartmentId};";
                }
                //add permission and worker-hours with default value-0 to extra workers if exists
                if (project.Permissions != null)
                    foreach (Permission permission in project.Permissions)
                    {
                        query += $"insert into task_management.permission(worker_id,project_id) values({permission.WorkerId}, {project.ProjectId});";
                        query += $"insert into task_management.worker_hours(project_id,worker_id,is_complete) values ({project.ProjectId} ,{permission.WorkerId},1);";
                    }
                //add worker-hours with default value-0 to all team workers //!!!!!!!!!!!!!!!!!!!!!!!!!!!
                List<User> teamWorkers = UserService.GetAllTeamUsers(project.TeamLeaderId);
                teamWorkers.ForEach(worker =>
                {
                    query += $"INSERT INTO task_management.worker_hours(project_id,worker_id,num_hours,is_complete) VALUES ({project.ProjectId},{worker.UserId},0,1);";
                });
                query += "commit;";
                int? x = DBAccess.RunNonQuery(query);
                return x >= 4;
        ...
        public static List<Project> GetProjectsByManager(int managerId)
        {
            try
            {
                string query = $"{GetProjectsQuery()} WHERE p.manager_id={managerId} ORDER BY project_name";
                List<Project> projectList = GetProjects(query);
                return projectList;
            }
            catch (Exception ex)
